...
def mocked_execvpe(
    monkeypatch, name, args, env, with_timeout=None, signal_type=signal.SIGINT
):
    ...
    try:
        ...
        if with_timeout:
            ...
            timer = threading.Timer(
                with_timeout, functools.partial(killer, process.pid)
            )
            timer.start()
        ...
        monkeypatch.setattr(
            "os.execvpe", functools.partial(mocked_execvpe, monkeypatch)
        )
...
def test_multiline(run, tmpenvdir, monkeypatch):
    ...
    monkeypatch.setattr(os, "execvpe", functools.partial(mocked_execvpe, monkeypatch))
    tmpenvdir.join("MULTI_LINE").write(
        """multi
line
"""
    )
...
def test_doesnt_exist(run, tmpdir, monkeypatch):
    ...
    result = (
        "Unable to run command" in response.value.message
        or "Unable to find command" in response.value.message
    )
    assert result
...
def test_error_code(run, tmpenvdir, monkeypatch):
    monkeypatch.setattr(os, "execvpe", functools.partial(mocked_execvpe, monkeypatch))
    with py.test.raises(Response) as response:
        run("envdir", str(tmpenvdir), "python", "-c", "import sys; sys.exit(19)")
...
@pytest.mark.skipif(
    py.std.sys.version_info < (2, 7) and platform.system() == "Windows",
    reason="os.kill doesn't work properly on " "Python < 2.7 on Windows",
)
def test_keyboard_interrupt(run, tmpenvdir, monkeypatch):
    monkeypatch.setattr(
        os,
        "execvpe",
        functools.partial(mocked_execvpe, monkeypatch, with_timeout=0.0000001),
    )
    with py.test.raises(Response) as response:
...
@pytest.mark.skipif(
    sys.platform == "win32", reason="Symlinks are not supported on windows"
)
